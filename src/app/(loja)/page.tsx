...
export default function HomeLoja() {
  ...
  // Estado de Controlo do Modal de Comparação
  ...
  return (
    <div 
      className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 relative pb-24"
    >
      
      {/* Carrossel Dinâmico de Banners via CMS (Snap-X Nativo) */}
      {configuracoes.banners_vitrine_urls.length > 0 && (
        <div 
          className="mb-12 flex w-full overflow-x-auto snap-x snap-mandatory gap-4 rounded-2xl shadow-lg custom-scrollbar bg-gray-100 border border-gray-200"
        >
      ...
      {carregando ? (
        ...
        <div 
          className="flex h-64 flex-col items-center justify-center rounded-2xl border-2 border-dashed border-gray-300 bg-gray-50 text-center"
        >
          <span 
              ...
              >
                ...
                >
                  {primeiraImagem ? (
                    ...
                    <span 
                      className="text-6xl opacity-30 grayscale transition-transform group-hover:scale-110 group-hover:opacity-100"
                    >
                      📱
                    </span>
                  )}
                  ...
                  {produto.saldo_estoque <= 3 && (
                    <span 
                      className="absolute left-3 top-3 rounded bg-red-600 px-2 py-1 text-xs font-black text-white shadow-md"
                    >
                ...
                >
                  ...
                  >
                    <h3 
                      className="mb-4 line-clamp-2 min-h-[3rem] text-sm font-bold text-gray-800 transition-colors group-hover:text-blue-700"
                    >
                      {produto.nome}
                    </h3>
                  </Link>
                  ...
                  >
                    ...
                    <p 
                      className="text-xs font-medium text-gray-500 line-through"
                    >
                    ...
                    >
                      ...
                      {configuracoes.desconto_pix_percentual > 0 && (
                        <span 
                          className="mb-1 rounded bg-green-100 px-1.5 py-0.5 text-[10px] font-bold text-green-700"
                        >
                    ...
                    >
                      ...
                      <label 
                        className="flex w-full items-center justify-center gap-2 rounded-xl border border-gray-300 py-2.5 text-xs font-bold text-gray-700 cursor-pointer hover:bg-gray-50 transition-colors"
                      >
                        <input 
                          type="checkbox" 
      ...
      {itensComparacao.length > 0 && (
        ...
        >
          <div 
            className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8 flex items-center justify-between"
          >
            <div 
              className="flex items-center gap-4"
            >
              <span 
                className="flex h-10 w-10 items-center justify-center rounded-full bg-blue-600 text-lg font-black shadow-inner"
              >
      ...
      {/* Renderização do Modal de Comparação */}
      <ModalComparacao 

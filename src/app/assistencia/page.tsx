...
export default function WorkspaceAssistencia() {
  ...
  useEffect(() => {
    ...
    const desinscrever = onSnapshot(
      ...
      (snapshot) => {
        ...
        // AÇÃO 1: VISÃO EM TÚNEL PARA TÉCNICOS
  ...
  // AÇÃO 3: FLUXO FISCAL - Avança de "Pronto para Retirada" para a Fila do Caixa (NFS-e)
  const lidarComFinalizacaoOS = async (idOs: string) => {
    ...
    try {
      ...
      await updateDoc(osRef, {
        status_atual: 'Aguardando NFS-e' // Status gatilho para o Módulo Caixa (Integração Wrapper)
      });
  ...
  const lidarComWhatsApp = (os: OrdemServico) => {
    ...
    const mensagens: Record<string, string> = {
      'Entrada/Check-list': `Olá ${os.dados_os.cliente_nome}, o seu ${os.dados_os.modelo_aparelho} deu entrada na nossa assistência We Have Resolve e passará por um check-list técnico.`,
      'Orçamento Pendente': `Olá ${os.dados_os.cliente_nome}! Já analisámos o seu ${os.dados_os.modelo_aparelho}. Por favor, entre em contacto connosco para lhe passarmos o orçamento.`,
      'Aprovado/Em Conserto': `Boas notícias, ${os.dados_os.cliente_nome}! O serviço no seu ${os.dados_os.modelo_aparelho} já está em andamento.`,
      'Pronto para Retirada': `Olá ${os.dados_os.cliente_nome}! O seu ${os.dados_os.modelo_aparelho} está pronto e a funcionar na perfeição. Pode vir levantá-lo na loja!`,
    };

    const texto = mensagens[os.status_atual] || `Olá ${os.dados_os.cliente_nome}, temos atualizações sobre o seu aparelho.`;
  ...
  return (
    <div className="flex h-screen flex-col font-sans bg-gray-50 p-6 overflow-hidden">
                    ...
                    osNaColuna.map((os) => (
                      <div key={os.id} className="rounded-lg border-l-4 bg-white p-4 shadow-sm transition hover:shadow-md border border-gray-100" style={{ borderLeftColor: os.cor_hexadecimal }}>
                        <div className="mb-2 flex justify-between items-start">
                          <div>
                            <p className="text-sm font-black text-gray-900 line-clamp-1">{os.dados_os.cliente_nome}</p>
                            <p className="mt-0.5 text-xs font-bold text-purple-700">{os.dados_os.modelo_aparelho}</p>
                          </div>
                        ...
                        <div className="flex flex-col gap-2 border-t border-gray-100 pt-3">
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] font-semibold uppercase text-gray-400 tracking-wider">Recepção: {os.auditoria.criado_por_nome}</span>
                            <button 

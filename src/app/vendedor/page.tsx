...
export default function WorkspaceVendedor() {
  const { usuarioAuth, usuarioDb, carregando: authCarregando } = useAuthStore();
  ...
  useEffect(() => {
    // Só inicia a busca se o estado de autenticação já resolveu
    ...
    // Busca apenas as comandas criadas pelo próprio vendedor logado
    const q = query(
      collection(bancoDeDados, 'comandas'),
      where('auditoria.criado_por_id', '==', usuarioAuth.uid)
    );
    ...
    const desinscrever = onSnapshot(
      ...
      (snapshot) => {
        ...
        // Ordenação em memória para evitar a obrigatoriedade de Índice Composto imediato no Firebase
        dados.sort((a, b) => {
      ...
      (err) => {
        ...
        setErro('Falha ao carregar suas métricas de vendas. Verifique a rede.');
        setCarregandoDados(false);
      }
    );

    return () => desinscrever();
  }, [usuarioAuth, authCarregando]);

  // Cálculo das Métricas de Performance
  const vendasExpressas = comandas.filter(c => c.fluxo_operacional === 'Venda Expressa');
  const totalRendimentoVendas = vendasExpressas.reduce((acc, curr) => acc + (curr.valor_total || 0), 0);
  ...
  return (
    <div className="flex h-screen w-full flex-col bg-gray-50 p-8 font-sans overflow-hidden">
      ...
      <header className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-200 pb-6">
        <div>
          <h1 className="text-3xl font-black text-gray-900">Meu Painel de Desempenho</h1>
          <p className="text-gray-500 mt-1">
            Olá, <strong className="text-blue-600">{usuarioDb?.nome_completo || 'Colaborador'}</strong>! Acompanhe seus resultados e histórico.
          </p>
        </div>
        <div className="flex items-center gap-2 rounded-full bg-blue-100 px-4 py-2 text-sm font-bold text-blue-800">
          <span>📊</span> Vendedor Ativo
        </div>
      </header>
      ...
      <div className="mb-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-xl border border-red-200 bg-white p-5 shadow-sm relative overflow-hidden">
          ...
          <p className="text-xs font-semibold text-red-500 mt-2">Registradas por você</p>
        </div>
              ...
              {carregandoDados ? (
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-sm font-medium text-gray-500">Buscando seu histórico...</p>
                    </div>
                  </td>
                </tr>
              ) : comandas.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center text-gray-400 font-medium">
                    Você ainda não registrou nenhuma operação no sistema.
                  </td>
                </tr>
              ) : (

...
export default function WorkspaceMetricas() {
  ...
  useEffect(() => {
    ...
    const desinscreverProdutos = onSnapshot(
      ...
      (err) => {
        console.error('[ERRO MÉTRICAS ESTOQUE]', err);
      }
    );
      ...
      <div className="flex h-screen w-full items-center justify-center bg-gray-100 p-8">
        <div className="flex max-w-md flex-col items-center text-center">
          ...
          <p className="mb-6 text-sm text-gray-500">
            Seu perfil não possui autorização executiva para visualizar as métricas globais.
          </p>
          <Link 
            href="/pdv" 
  ...
  const capitalEstoque = produtos
    .reduce((acc, curr) => acc + (curr.preco * curr.saldo_estoque), 0);

  const osAtivas = comandas
    .filter(c => c.fluxo_operacional === 'Assistência Técnica' && c.status_atual !== 'Pronto para Retirada')
    .length;
        ...
        <header className="mb-8 shrink-0 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-200 pb-6">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Dashboard Executivo de Faturamento e Operações.
            </p>
          </div>
        </header>

        {erro && (
          <div className="mb-4 shrink-0 rounded border-l-4 border-red-500 bg-red-50 p-4 font-semibold text-red-700 shadow-sm">
            ⚠️ {erro}
          </div>
        )}
            ...
            <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
              <div className="bg-gray-50 p-4 border-b border-gray-200">
                <h2 className="text-lg font-bold text-gray-800">
                  Ranking de Vendedores (Receita Gerada)
                </h2>
              </div>

...
export default function ModalEditarProduto({ aberto, produto, aoFechar }: ModalEditarProdutoProps) {
  ...
  useEffect(() => {
    if (aberto && produto) {
      ...
      setSaldoFisico(produto.saldo_estoque.toString());
      setErro(null);
    }
  }, [aberto, produto]);
  ...
  const lidarComEdicao = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateDoc(docRef, {
        ...
        'auditoria.atualizado_por': usuarioDb?.nome_completo || usuarioAuth?.uid,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 font-sans backdrop-blur-sm transition-opacity">
      <div className="w-full max-w-lg rounded-xl bg-white shadow-2xl overflow-hidden border border-indigo-500">
        ...
        {erro && (
          <div className="bg-red-50 p-4 border-b border-red-200 text-sm font-semibold text-red-700 break-words">⚠️ {erro}</div>
        )}

        <form onSubmit={lidarComEdicao} className="p-6 space-y-4">
          <div>
